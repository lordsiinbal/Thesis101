...
class det:
    ...
    def __init__(self, opt, source, window):
        ...
        self.opt.half &= (pt or jit or onnx or engine) and self.device.type != 'cpu'
        if pt or jit:
    ...
    def detect(self):
        ...
        if self.window.getViolationRecord:
            self.violationIDLatest = str(self.window.getViolationRecord[len(
                self.window.getViolationRecord) - 1]['violationID']).split("-")
            self.violationIDLatest = int(self.violationIDLatest[1])
        else:
            self.violationIDLatest = 0
        for frame_idx, (path, img, im0s, vid_cap, s, frm_id, vid_fps, ret, tim) in enumerate(self.dataset):
            if not self.stopped:
                ...
                # Inference
                self.opt.visualize = increment_path(self.save_dir / Path(path).stem,
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    if self.webcam:  # batch_size >= 1
                        p, im0, frame = path[0], im0s.copy(), self.dataset.count
                        s += f'{i}: '
                    else:
                    ...
                    if len(det):
                        ...
                        if frame_idx > 0 or self.PREV_BB.any() != 0:
                            t8 = time_sync()
                            xy, xywhs, confs, clss, self.PREV_XY, self.PREV_BB, self.time_start = isStationary(
                                    xy, wh, xywhs, confs, clss, self.PREV_XY, vid_fps, frm_id, bbox, self.PREV_BB, self.time_start)
                            t9 = time_sync()
                            ...
                            if len(xy) > 0:
                                ...
                                self.dt[4] += t5 - tim
                                LOGGER.info(
                                    f'Done. Read-frame: ({t1-tim:.3f}), YOLO:({t3 - t2:.3f}s), Tracker:({t5 - t4:.3f}s), Stationary:({t9 - t8:.3f}s), isInsideROI: ({t12-t11:.3f}s) Overall:({t5-tim:.3f}s)')
                        else:
                            self.PREV_BB = bbox
                            self.PREV_XY = xy
                ...
                LOGGER.info(
                    f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS, %.1fms deep sort update per image at shape {(1, 3, *self.imgsz)}. \nAverage speed of %.1fms per detection' % t)
                raise StopIteration
